...
/* static members */
UIRoot *StateLevelPackSideView::m_sGUI = NULL;

StateLevelPackSideView::StateLevelPackSideView(LevelsPack *pActiveLevelPack,
                                           bool drawStateBehind,
                                           bool updateStatesBehind)
  : StateMenu(drawStateBehind, updateStatesBehind) {
  m_name = "StateLevelPackSideView";
...
StateLevelPackSideView::~StateLevelPackSideView() {
...
void StateLevelPackSideView::enter() {
...
void StateLevelPackSideView::enterAfterPop() {
...
void StateLevelPackSideView::checkEvents() {
...
void StateLevelPackSideView::switchtoFavorites() {
  UILevelList *pList =
    reinterpret_cast<UILevelList *>(m_GUI->getChild("FRAME:LEVEL_LIST"));
  std::string v_id_level = pList->getSelectedLevel();
...
void StateLevelPackSideView::executeOneCommand(std::string cmd,
                                               std::string args) {
...
void StateLevelPackSideView::xmKey(InputEventType i_type, const XMKey &i_xmkey) {
...
void StateLevelPackSideView::clean() {
  if (StateLevelPackSideView::m_sGUI != NULL) {
    delete StateLevelPackSideView::m_sGUI;
    StateLevelPackSideView::m_sGUI = NULL;
  }
}

void StateLevelPackSideView::createGUIIfNeeded(RenderSurface *i_screen) {
  if (m_sGUI != NULL)
    return;
  ...
  unsigned int v_offsetX = width / 2;
  unsigned int v_offsetY = height / 32;
  unsigned int frameWidth = width - v_offsetX;
  ...
  UIStatic *pLevelPackSideViewTitle = new UIStatic(v_frame,
                                                 0,
                                                 0,
                                                 "(level pack name goes here)",
                                                 v_frame->getPosition().nWidth,
                                                 40);
  pLevelPackSideViewTitle->setID("VIEWER_TITLE");
  pLevelPackSideViewTitle->setFont(drawLib->getFontMedium());
...
void StateLevelPackSideView::updateLevelsList() {
  UILevelList *pList =
    reinterpret_cast<UILevelList *>(m_GUI->getChild("FRAME:LEVEL_LIST"));
...
void StateLevelPackSideView::updateGUI() {
  UIStatic *pTitle =
...
void StateLevelPackSideView::updateInfoFrame() {
  UILevelList *v_list =
    reinterpret_cast<UILevelList *>(m_GUI->getChild("FRAME:LEVEL_LIST"));
...
std::string StateLevelPackSideView::getInfoFrameLevelId() {
  UILevelList *v_list =
    reinterpret_cast<UILevelList *>(m_GUI->getChild("FRAME:LEVEL_LIST"));
  return v_list->getSelectedLevel();
}

void StateLevelPackSideView::updateRights() {
  UIButton *v_button;
  UILevelList *v_list =
    reinterpret_cast<UILevelList *>(m_GUI->getChild("FRAME:LEVEL_LIST"));
  std::string v_id_level = v_list->getSelectedLevel();
